...
class DeviceMonitor {
  ...
  private detectMode() {
    if (typeof window !== 'undefined' && window.electronAPI) {
        ...
        // Default to web, but if simulation is off, check if we might want local server
        this.currentMode = 'WEB_ONLY';
  ...
  public setSimulationEnabled(enabled: boolean) {
      ...
      if (enabled) {
          ...
          this.detectMode();
          // If detection fell back to WEB_ONLY, but we have a custom server URL configured in settings component (conceptually),
          // we might want LOCAL_SERVER. For now, rely on default detection.
          if (this.currentMode === 'WEB_ONLY' && this.localServerUrl) {
               // We will try to fetch in the tick, if it fails, it stays disconnected
               this.currentMode = 'LOCAL_SERVER';
          }
      }
      this.restart();
  ...
  private restart() {
      ...
      this.timer = window.setInterval(() => this.tick(), 2000) as unknown as number; 
  }
  ...
  private async tickLocalServer() {
      try {
          ...
          this.isConnected = false;
          // Silent fail to avoid console spam, or maybe switch back to Web Only
      }
  }
  ...
  private tickWeb() {
      if (document.hidden) {
          ...
          this.handleActivityUpdate('Browser', document.title, { url: window.location.href });
      }
  }
  ...
  private tickSimulation() {
      ...
      const simulatedApps = [
        { name: 'VSCode', detail: 'IoT Project / src / main.cpp', meta: { pid: 1001 } },
        { name: 'Chrome', detail: 'StackOverflow - React Hooks', meta: { url: 'https://stackoverflow.com...' } },
        { name: 'WeChat', detail: 'Chat with Product Manager', meta: { pid: 2045 } },
        { name: 'Terminal', detail: 'npm install', meta: { pid: 8872 } }
      ];
  ...
  private flushSession(endTime: number) {
      ...
      if (durationMinutes >= 1) {
          ...
          if (this.currentSession.metadata?.url) {
              desc += `\n${this.currentSession.metadata.url}`;
          }
  ...
  private getDeviceSource(): 'desktop' | 'mobile' | 'tablet' {
      ...
      if (this.currentMode === 'WEB_ONLY') return 'desktop';
      return 'desktop';
  }
  ...
  private saveLogToStorage(newLog: TimeBlock) {
    ...
    if (lastLog && 
        ...
        if (gap <= 2 && gap >= -5) {
            ...
            // Persist below will save the mutation
        } else {
  ...
  private getAppColor(appName: string): string {
    ...
    if (name.includes('code') || name.includes('dev')) return '#007acc';
    if (name.includes('chrome') || name.includes('edge') || name.includes('firefox')) return '#eab308';
    if (name.includes('chat') || name.includes('slack') || name.includes('discord') || name.includes('wechat')) return '#10b981';
    if (name.includes('music') || name.includes('spotify')) return '#1db954';
    if (name.includes('video') || name.includes('youtube')) return '#ef4444';
    if (name.includes('term') || name.includes('console')) return '#1f2937';
