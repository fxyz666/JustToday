...
import { CheckSquare, Square, XSquare, Crosshair, Edit2, Trash2, Loader2, Cloud, Check, Smartphone, Monitor, Tablet, User, Target, AlertTriangle, Layers, FileJson, Save as SaveIcon, Trash, ChevronDown, RefreshCw, AlertCircle } from 'lucide-react';
...
const SchedulePanel: React.FC<Props> = ({ tasks, selectedDate, isToday, onTaskUpdate, onTaskCreate, onTaskEdit, onTaskDelete, onCustomDrop, onSyncCloud, draggingMode, lang, theme = 'light', globalDragItem, dragPosition, templates, onOpenTemplateModal, onLoadTemplate, onDeleteTemplate }) => {
  ...
  const [syncStep, setSyncStep] = useState<'downloading' | 'merging' | 'uploading' | null>(null);
  ...
  const handleUnifiedSync = async () => {
      ...
      try {
          ...
          pendingTimeouts.push(successTimeout);
      } catch (e: any) {
          console.error('Sync error:', e);
          setSyncStatus('error');
          setSyncErrorMsg(e.message || t('syncFailed', lang));
          ...
          pendingTimeouts.push(errorTimeout);
      } finally {
          // Clean up old timeouts after a safe period
          setTimeout(clearTimeouts, 6000);
      }
  };
  ...
  const renderSyncButton = () => {
      ...
      let btnClass = "hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-300 border border-transparent";
      let text = t('syncData', lang);
      let Icon = Cloud;

      if (isSyncing) {
          btnClass = "bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300 border border-blue-100 dark:border-blue-800 shadow-sm shadow-blue-100 dark:shadow-blue-900/20";
          text = t('syncingCloud', lang);
          Icon = RefreshCw;
      } else if (isSuccess) {
          btnClass = "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800 shadow-sm shadow-emerald-100 dark:shadow-emerald-900/20";
          text = newItemsCount > 0 ? `${newItemsCount} ${t('newItems', lang)}` : t('upToDate', lang);
          Icon = Check;
      } else if (isError) {
          btnClass = "bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400 border border-red-100 dark:border-red-800 shadow-sm shadow-red-100 dark:shadow-red-900/20";
          text = t('syncFailed', lang);
          Icon = AlertCircle;
      }
      ...
      return (
        <div className="relative">
            <button
                ...
                disabled={isLoading}
                className={`flex items-center gap-2 px-2.5 py-1.5 rounded-md text-xs font-medium transition-all duration-200 ${isLoading ? 'opacity-70 cursor-not-allowed' : 'hover:scale-105 active:scale-95'} ${btnClass}`}
                title={isError ? syncErrorMsg : (lang === 'zh-CN' ? "同步到云端" : "Sync with Cloud")}
            >
                ...
                <span className="hidden sm:inline whitespace-nowrap font-medium">{text}</span>
            </button>

            {/* Error Tooltip with improved positioning */}
            {isError && syncErrorMsg && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-3 py-2 bg-red-600 text-white text-xs rounded-lg shadow-xl z-[100] max-w-[250px] whitespace-normal break-words animate-in fade-in slide-in-from-top-1 duration-200">
