...
const StatsChart: React.FC<Props> = ({ tasks, lang, range, selectedDate, theme = 'light', chartType, onChartChange }) => {
  ...
  // Calculate Peak Hour Logic
  const getHourlyDistributionData = () => {
      const buckets = new Array(24).fill(0);
      getActuals().forEach(t => {
          // Add duration to the start hour bucket
          // For higher accuracy, we could split duration across hours, but this is sufficient for trends
      ...
      return { xAxis, data, rawBuckets: buckets };
  };
  ...
  const summaryMetrics = useMemo(() => {
      ...
      if (chartType === 'apps') {
          ...
          topApp = topApp.split(' - ')[0].substring(0, 15);
      ...
      if (chartType === 'hours') {
          // Calculate Real Peak Hour
          const { rawBuckets } = getHourlyDistributionData();
          let maxMins = 0;
          let peakHourIndex = -1;
          rawBuckets.forEach((mins, idx) => {
              if (mins > maxMins) {
                  maxMins = mins;
                  peakHourIndex = idx;
              }
          ...
          const peakHourStr = peakHourIndex >= 0 
          ...
          return [
              ...
              { label: t('peakHour', lang), value: peakHourStr, icon: Hourglass, color: 'text-pink-500' }
          ];
      }
  ...
  const getActivitiesData = () => {
    ...
    actuals.forEach(t => {
      // Clean title for aggregation
      let key = t.title.split(' - ')[0].trim(); 
  ...
  const renderChart = () => {
    ...
    if (chartType === 'apps' || chartType === 'capital') {
        ...
        const data = isCapital ? getCapitalData() : getActivitiesData();
