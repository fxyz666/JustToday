...
import { LayoutDashboard, Calendar, Settings, Smartphone, Laptop, Tablet, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Calendar as CalendarIcon, BarChart2, Sun, Moon, Languages, Target, Inbox, ListTodo, FileJson, Save as SaveIcon, X, Plus, LayoutList, CheckCircle, XCircle, Info, AlertTriangle } from 'lucide-react';
...
import { useNotifications, notificationService, Notification, NotificationType } from './services/notificationService';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Ensure webDavConfig has loginMode for backward compatibility
        if(parsed.webDavConfig && !parsed.webDavConfig.loginMode) {
            parsed.webDavConfig.loginMode = 'manual';
        }
  ...
  const handleSyncCloud = async (): Promise<number> => {
    try {
        const { state: syncedState, changes } = await syncService.syncToCloud(state, language);
  ...
  const handleTaskUpdate = (updatedTask: TimeBlock) => {
    setState(prev => {
      let newTasks = prev.tasks.map(t => {
          if (t.id === updatedTask.id) return { ...updatedTask, updatedAt: Date.now() };
  ...
  const handleOpenTemplateModal = () => {
      ...
      if (!hasPlan) {
          notificationService.info(t('noPlansToSave', language));
          return;
      }
      setTemplateName('');
  ...
  // Fixed handleGoalSave to recalculate progress immediately
  const handleGoalSave = (goal: Goal) => {
      const timestamp = Date.now();
      setState(prev => {
          let newGoals;
          if (goal.id) {
              newGoals = prev.goals.map(g => g.id === goal.id ? { ...goal, updatedAt: timestamp } : g);
          } else {
              const newGoal = { ...goal, id: generateId(), updatedAt: timestamp };
              newGoals = [...prev.goals, newGoal];
          }
          // IMPORTANT: Recalculate goals immediately to ensure changes (e.g. unit type) are reflected
          const recalculatedGoals = recalculateGoals(newGoals, prev.tasks);
          return { ...prev, goals: recalculatedGoals };
      });
  ...
  const getToastBg = (type: NotificationType) => {
    switch (type) {
      case 'success': return 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-900/30';
      case 'error': return 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-900/30';
      case 'warning': return 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-900/30';
      default: return 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-900/30';
    }
  };
    ...
    <div className={`flex flex-col md:flex-row w-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100 overflow-hidden font-inter transition-colors duration-300 h-[100dvh]`}>
      ...
      <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-[9999] flex flex-col gap-2 pointer-events-none">
        {notifications.map((notification, index) => (
          <div
            key={notification.id}
            className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg border min-w-[280px] max-w-[90vw] animate-in slide-in-from-bottom-2 ${getToastBg(notification.type)}`}
            style={{ animationDelay: `${index * 50}ms` }}
          >
            {getToastIcon(notification.type)}
            <span className="flex-1 text-sm font-medium text-gray-800 dark:text-gray-200">{notification.message}</span>
            <button
              onClick={() => removeNotification(notification.id)}
              className="p-1 hover:bg-black/5 dark:hover:bg-white/10 rounded-full transition-colors pointer-events-auto"
            >
      ...
      <aside className="hidden md:flex w-20 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex-col items-center py-6 gap-6 z-20 shadow-sm shrink-0 transition-colors duration-300">
        ...
        <div className="mt-auto flex flex-col gap-4 items-center w-full">
           {/* Sidebar language/theme toggles kept as quick access */}
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative pt-[calc(max(env(safe-area-inset-top),24px)+2px)] md:pt-0">
        ...
        {/* HEADER - Date Navigation and Theme controls */}
        {(view === 'schedule' || view === 'stats') && (
            <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-3 py-1 md:px-6 md:py-3 flex items-center justify-between shrink-0 z-[100] transition-colors duration-300 relative select-none min-h-[44px] md:min-h-[56px] gap-2">
              ...
              {/* CENTER: Date Navigation - Explicitly above other containers */}
              <div className="flex-1 md:flex-none flex md:absolute md:left-1/2 md:top-1/2 md:transform md:-translate-x-1/2 md:-translate-y-1/2 items-center justify-center gap-2 md:gap-4 z-40 min-w-0">
                 <button 
                    onClick={() => setSelectedDate(prev => addDays(prev, -1))} 
                    className="p-1 md:p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-gray-900 dark:text-gray-500 dark:hover:text-gray-200 transition-colors active:scale-90 shrink-0 pointer-events-auto"
                 >
                    <ChevronLeft size={20} strokeWidth={2.5}/>
                 </button>
                 
                 <div className="relative group flex flex-col items-center justify-center cursor-pointer min-w-0 pointer-events-auto">
                    <div className="px-2 md:px-3 py-1 rounded-xl group-hover:bg-gray-50 dark:group-hover:bg-gray-700/50 transition-colors truncate max-w-[120px] sm:max-w-none text-center">
                        <h2 className="text-sm md:text-lg font-extrabold text-gray-800 dark:text-gray-100 leading-none whitespace-nowrap tracking-tight truncate">
